...
MODULE Cu_Grellens_CPTEC
 ...
 USE Parallelism, ONLY: &
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: RunCu_GrellEnsCPTEC
  PUBLIC :: Init_Cu_GrellEnsCPTEC
...
CONTAINS
  SUBROUTINE Init_Cu_GrellEnsCPTEC()
    !
...
!wcrp2018
  ...
  END SUBROUTINE Init_Cu_GrellEnsCPTEC

  SUBROUTINE RunCu_GrellEnsCPTEC(&
       ps     ,sl     ,ua     ,va     ,omg    ,t2     ,tn1    , &
       q2     ,qn1    ,zz     ,xland  ,dtime  ,RAINCV , &
       kuo    ,ktop   ,kbot   ,plcl   ,qliq   ,nCols  ,kMax)
...
!    ter11(1:nCols) = zz(1:nCols)
    ...
    bncy   =0.0_r8  !buoancy
    cape   =0.0_r8  !work-function

    CALL grellens2(&
         t      ,q      ,tn     ,qo     ,po     ,p      ,psur   ,us     , &
         vs     ,omeg   ,zz     ,dtime  ,outt   ,outq   ,pre1   ,ktop   , &
         kbot   ,nCols  ,kMax   ,xland  ,qliq   ,cupclw ,massfln,xmb,bncy,cape  , &
         ierr)
    !
    ...
    RETURN
  END SUBROUTINE RunCu_GrellEnsCPTEC

  !*-------------------------
  SUBROUTINE grellens2(& 
       t      ,q      ,tn     ,qo     ,po     ,p      ,psur   ,us     , &
       vs     ,omeg   ,z1     ,dtime  ,outt   ,outq   ,pre    ,ktop   , &
       kbcon  ,nCols  ,kMax    ,mask  ,qliq   ,cupclw ,massfln,xmb,bncy,wf_cape,&
       ierr  )
    IMPLICIT NONE
    !
    ! input variables
    ...
    REAL(KIND=r8)   , INTENT(INOUT)       :: xmb(nCols)
    ...
    !new
     ...
     REAL(KIND=r8)   , INTENT(INOUT)      :: bncy   (nCols,kMax)
     REAL(KIND=r8)   , INTENT(INOUT)      :: wf_cape(nCols)
    !
    ! LOCAL VARIABLES
    !
    INTEGER            :: i
    ...
    DO k=1,kMax
       DO i=istart,iend
          IF( ierr(i) == 0 .AND. kstabm(i)-1 >= kstabi(i) .AND. &
             ...
             IF(iens >  4)THEN
                cd(i,k)=cd(i,k-1)+REAL(iens-4,kind=r8)*entr_rate &
...
!-------
    ...
    IF (nCols_gz > 0 ) THEN
       ...
       CALL Ensemble(&
            ...
            p_cup_gz      )  !REAL   ,(IN   )(nCols,kMax        )             p_cup           (1:nCols   ,1:kMax)  )        !REAL        ,(IN   )(nCols,kMax           )
...
!        if(maxens22.eq.1)edtc(i,k) = edt(i)*0.25_r8
    ...
    DO k=1,maxens22
       DO i=istart,iend
          IF(ierr(i) == 0)THEN
             edtc(i,k)=-edtc(i,k)*pwav(i)/MAX(pwev(i),10.0e-12_r8)
             !
