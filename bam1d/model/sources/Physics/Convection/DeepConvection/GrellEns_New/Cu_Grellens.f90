...
MODULE Cu_Grellens
 ...
 USE Parallelism, ONLY: &
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: Init_Cu_GrellEns
  PUBLIC :: RunCu_GrellEns
...
CONTAINS
  ...
  END SUBROUTINE Init_Cu_GrellEns

  SUBROUTINE RunCu_GrellEns(&
...
!      if (dodia(nDiag_deepql ))  call updia(omeg                  ,nDiag_deepql   ,latco,jdt)
  ...
  SUBROUTINE grellens2(& 
       ...
       lon    ,tke    ,cape   ,cine   ,sens   ,ierr   ,outt   ,outq  ,outqc    , &
    ...
    REAL(KIND=r8)   , INTENT(INOUT)       :: cape (nCols)
...
!------------
    ...
    DO i=istart,iend
       cine(i)=cine1(i)
       dd=ucape1(i)-ucape0(i)
       IF(ucape1(i)>0.0_r8.AND.dd>0.0_r8)THEN
       d_ucape(i)=dd
       ELSE
       d_ucape(i)=0.0_r8
       END IF
    ...
    DO i=istart,iend
       dd=dcape1(i)-dcape0(i)
       IF(dcape1(i)>0.0_r8.AND.dd>0.0_r8)THEN
       d_dcape(i)=dd
       ELSE
       d_dcape(i)=0.0_r8
       END IF
...
SUBROUTINE cup_up_CIN( &
    ...
    REAL(KIND=r8), INTENT(IN   ) :: t_cup     (nCols,kMax)
    REAL(KIND=r8), INTENT(IN   ) :: dby       (nCols,kMax)
    REAL(KIND=r8), INTENT(INOUT) :: ucape     (nCols)
...
!--------------
    ...
    DO  k=2,kMax-1
       DO  i=istart,iend
          IF(ierr(i) == 0 .AND. k >kk1(i) .AND. k <= ktop(i) ) THEN
             dz=z(i,k)-z(i,k-1)
             da=dz*(9.81_r8/(1004.0_r8*(   &
                  (t_cup(i,k)))))*dby(i,k-1)/ &
                  (1.0_r8+gamma_cup(i,k))
...
!        if(maxens22.eq.1)edtc(i,k) = edt(i)*0.25_r8
    ...
    DO k=1,maxens22
       DO i=istart,iend
          IF(ierr(i) == 0)THEN
             edtc(i,k)=-edtc(i,k)*pwav(i)/MAX(pwev(i),1.0e-12)
             !
  ...
  SUBROUTINE cup_forcing_ens( &
       ...
       xfmax,maxens22,den1,cape,d_ucape,d_dcape,cine,sens,tkeMax,tkeMedia )
...
!niloc
!----------------------------------------------------------------
!four new closures    nilo 19 jan 2013
! USE CAPE/CINE/TKEMAX
!xff_ens3=den1(i,1)*Const1*tkeMax(i)**0.5_r8
...
!--------------------------------------------------------------
             ...
             xff_ens3(i,17)=den1(i,1)*0.10_r8*ee*cc  !Turbulence+cine impact
             xff_ens3(i,18)=cape(i)/dtime           !dcape trigge
             xff_ens3(i,19)=d_ucape(i)/dtime     ! dcape trigger        
             xff_ens3(i,20)=d_dcape(i)/dtime
             
             xff_ens3(i,17)=MAX(0.0_r8,xff_ens3(i,17))
             xff_ens3(i,18)=MAX(0.0_r8,xff_ens3(i,18))
             xff_ens3(i,19)=MAX(0.0_r8,xff_ens3(i,19))
             xff_ens3(i,20)=MAX(0.0_r8,xff_ens3(i,20))
!----------------------------------------------------------end 
     END IF
   END DO
                ...
                IF(iresulte == 1)THEN
                   ...
                   IF(xff0(i) >  xff_max)THEN 
                      ...
                      xf(i,nall(i,nens)+16)=MAX(0.0_r8,-xff_ens3(i,16)/xk(i,nens)) &
                           +massfld
                      xf(i,nall(i,nens)+17)=MAX(0.0_r8,xff_ens3(i,17)+massfld)
