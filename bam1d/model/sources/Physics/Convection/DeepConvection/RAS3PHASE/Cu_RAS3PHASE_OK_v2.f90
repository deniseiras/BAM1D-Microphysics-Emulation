...
!    INTEGER      , INTENT(OUT  ) ::  kuo  (nCols)
    ...
    REAL(KIND=r8) :: qgrs (nCols,kMax,3+ntrac)!	  qgrs     - real, layer mean tracer concentration     ix,levs,ntrac!
    REAL(KIND=r8) :: qliq (nCols,kMax)!     qgrs     - real, layer mean tracer concentration	 ix,levs,ntrac!
    REAL(KIND=r8) :: qice (nCols,kMax)!     qgrs     - real, layer mean tracer concentration	 ix,levs,ntrac!
...
!     real (kind=r8), parameter :: rhc_max = 0.999       ! for pry
    ...
    REAL(KIND=r8) :: clw(nCols,kMax,2+ntrac)
...
!  --- ...  array to track fraction of "cloud" in the form of ice
    ...
    END DO
        ...
        call rascnv(nCols,     nCols,    kMax,   2*dt, 2*dt, rannum,         &
                 tgrs ,    qgrs(:,:,1:1),   ugrs,    vgrs, clw,tottracer ,&
    ...
    DO k=1,kMax
       DO i=1, nCols
            IF(rain1(i) > 0.0_r8)THEN
              ...
              ql3(i,k)= clw  (i,k,2)!ql3(i,k) + (clw  (i,k,2) - qliq(i,k))/(2*dt)
              qi3(i,k)= clw  (i,k,1)!qi3(i,k) + (clw  (i,k,1) - qice(i,k))/(2*dt)
            END IF   
       END DO      
    END DO
        !  IF(nClass>0 .and. PRESENT(gvarm))THEN
...
!      deallocate ( clw )
  ...
  SUBROUTINE rascnv(IM,    IX,     k,      dt,    dtf,  rannum      &
    ...
    INTEGER      , INTENT(IN   ) :: IM!     ix, im   - integer, horiz dimention and num of used pts      1    !
    INTEGER      , INTENT(IN   ) :: IX!     ix, im   - integer, horiz dimention and num of used pts      1    !
    ...
    REAL(kind=r8), INTENT(IN   ) :: rannum(ix,nrcm)
    REAL(kind=r8), INTENT(INOUT) :: tin(ix,k)
    REAL(kind=r8), INTENT(INOUT) :: qin(ix,k)
    REAL(kind=r8), INTENT(INOUT) :: uin(ix,k)
    REAL(kind=r8), INTENT(INOUT) :: vin(ix,k)
    REAL(kind=r8), INTENT(INOUT) :: ccin(ix,k,trac+2)
    REAL(kind=r8), INTENT(IN   ) :: prsi(ix,k+1) 
    REAL(kind=r8), INTENT(IN   ) :: prsik(ix,k+1)
    REAL(kind=r8), INTENT(IN   ) :: prsl(ix,k)
    REAL(kind=r8), INTENT(IN   ) :: prslk(ix,k+1)
    REAL(kind=r8), INTENT(IN   ) :: phil(ix,k)
    REAL(kind=r8), INTENT(IN   ) :: phii(ix,k+1)
    INTEGER      , INTENT(IN   ) :: KPBL(im)
    REAL(kind=r8), INTENT(IN   ) :: CDRAG(im)

    REAL(kind=r8), INTENT(OUT  ) :: RAINC(im) 
    INTEGER      , INTENT(OUT  ) ::  kbot(im)
    INTEGER      , INTENT(OUT  ) ::  ktop(im)
    INTEGER      , INTENT(OUT  ) ::  kuo(im)
    REAL(kind=r8), INTENT(INOUT) :: DDVEL(im)
    LOGICAL      , INTENT(IN   ) :: FLIPV
    REAL(kind=r8), INTENT(IN   ) :: facmb
    REAL(kind=r8), INTENT(IN   ) :: garea(im)
    INTEGER      , INTENT(IN   ) :: lmh(im)
    REAL(kind=r8), INTENT(IN   ) :: ccwfac(im)  
    REAL(kind=r8), INTENT(IN   ) :: rhc(im,k)
    REAL(kind=r8), INTENT(OUT  ) :: ud_mf(ix,k)
    REAL(kind=r8), INTENT(OUT  ) :: dd_mf(ix,k)
    REAL(kind=r8), INTENT(OUT  ) :: det_mf(ix,k)
    LOGICAL      , INTENT(IN   ) :: lprnt
    LOGICAL      , INTENT(IN   ) :: CALKBLMsk(ix)
