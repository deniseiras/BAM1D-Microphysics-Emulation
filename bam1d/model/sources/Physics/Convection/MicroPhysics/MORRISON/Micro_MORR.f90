...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  PUBLIC :: Init_Micro_MORR
  PUBLIC :: RunMicro_MORR
CONTAINS


!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  SUBROUTINE Init_Micro_MORR()
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  END SUBROUTINE Init_Micro_MORR
    !+---+-----------------------------------------------------------------+
   
   SUBROUTINE RunMicro_MORR( &
...
!    REAL(KIND=r8), INTENT(IN   ) :: ht( 1:nCols  )
    ...
    QI1D = 0.0_r8;    QR1D = 0.0_r8;    NI1D = 0.0_r8;    NS1D = 0.0_r8
...
!       DO j=jts,jte      ! j loop (north-south)
          ...
          ELSE
             DO k = 1, kMax
                nc1d(k)=0.0_r8 ! temporary placeholder, set to constant in microphysics subroutine
                iinum=1
             ENDDO
          ENDIF
          ...
          DO k=1,kMax
             ...
             !          EFFCS(I,K)     = MIN(EFFC(I,K),50.)
             !          EFFCS(I,K)     = MAX(EFFCS(I,K),1.)
...
!       END DO
  ...
  SUBROUTINE MORR_TWO_MOMENT_MICRO(&
    ...
    !REAL(KIND=r8) ::  W3D     (1:kMax)    ! GRID-SCALE VERTICAL VELOCITY (M/S)
    !REAL(KIND=r8) ::  WVAR    (1:kMax)    ! SUB-GRID VERTICAL VELOCITY (M/S)
    ! below for wrf-chem
    ...
    DO K = 1,kMax
       ...
       IF (T3D(K).GE.273.15_r8) THEN
          ...
          IF (QR3D(K).GE.1.E-8_r8.AND.QNI3D(K).GE.1.E-8_r8) THEN
             ...
             PRACS(K) = CONS31*(((1.2_r8*UMR-0.95_r8*UMS)**2+              &
