...
MODULE Micro_HugMorr
  ...
  INTEGER,     PARAMETER  :: PBL=2   !Kubota teste para usar tke PBL=1
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE RunMicro_HugMorr( &
    ...
    ! ******* Denis WRITE input vars CSV ****************************

    ! Saves at each 18 min 
   if ( MOD(dt, 720.0_r8) == 0.0_r8 ) then
   do k=1, kMax
      call csv_write(9876, k, .false.)
      ! call csv_write(9876, si(k), .false.)
      call csv_write(9876, sl(k), .false.)
      call csv_write(9876, Tc(1,k), .false.)
      call csv_write(9876, qv(1,k), .false.)
      call csv_write(9876, qc(1,k), .false.)
      call csv_write(9876, qr(1,k), .false.)
      call csv_write(9876, qi(1,k), .false.)
      call csv_write(9876, qs(1,k), .false.)
      call csv_write(9876, qg(1,k), .false.)
      call csv_write(9876, ni(1,k), .false.)
      call csv_write(9876, ns(1,k), .false.)
      call csv_write(9876, nr(1,k), .false.)
      call csv_write(9876, NG(1,k), .false.)
      call csv_write(9876, NC(1,k), .false.)
      call csv_write(9876, tke(1,k), .false.)
      call csv_write(9876, omega(1,k), .true.)
   enddo
   endif
    ...
    ! ******* Denis WRITE Output vars CSV ****************************
    ! Saves at each 18 min 
   if ( MOD(dt, 720.0_r8) == 0.0_r8 ) then
   do k=1, kMax
      call csv_write(8456, k, .false.)
      call csv_write(8456, Tc(1,k), .false.)
      call csv_write(8456, qv(1,k), .false.)
      call csv_write(8456, qc(1,k), .false.)
      call csv_write(8456, qr(1,k), .false.)
      call csv_write(8456, qi(1,k), .false.)
      call csv_write(8456, qs(1,k), .false.)
      call csv_write(8456, qg(1,k), .false.)
      call csv_write(8456, ni(1,k), .false.)
      call csv_write(8456, ns(1,k), .false.)
      call csv_write(8456, nr(1,k), .false.)
      call csv_write(8456, NG(1,k), .false.)
      call csv_write(8456, NC(1,k), .true.)
   end do
   call csv_write(8456, LSRAIN(1:1), .false.)
   call csv_write(8456, LSSNOW(1:1), .true.)
   endif 
                ...
                IF (DUM.GE.0.001_r8) THEN
                   ...
                   ELSE IF (IBASE.EQ.2) THEN
                      ...
                      ELSE IF (IACT.EQ.2) THEN
                         ...
                         ETA1 = (ALPHA*DUM/GG)**1.5_r8/(2.0_r8*PI*RHOW*GAMM*NANEW1)
                         ETA2 = (ALPHA*DUM/GG)**1.5_r8/(2.0_r8*PI*RHOW*GAMM*NANEW2)
                         !print*, 'Enver Inside HugMorr', AACT, RM1, AACT/(3.0_r8*RM1), (AACT/(3.0_r8*RM1))**1.5_r8
                         SM1 = 2.0_r8/BACT**0.5_r8*(AACT/(3.0_r8*RM1))**1.5_r8
                         SM2 = 2.0_r8/BACT**0.5_r8*(AACT/(3.0_r8*RM2))**1.5_r8
                         !print*, 'Enver Inside HugMorr', AACT, RM2, AACT/(3.0_r8*RM2), (AACT/(3.0_r8*RM2))**1.5_r8
                         print*, 'Enver Inside HugMorr', F11, PSI, ETA1, F11*(PSI/ETA1)**1.5_r8, (SM1**2/(ETA1+3.0_r8*PSI))**0.75_r8
                         print*, 'Enver Inside HugMorr', F12, PSI, ETA2, F12*(PSI/ETA2)**1.5_r8, (SM2**2/(ETA2+3.0_r8*PSI))**0.75_r8
                         !print*, 'Enver Inside HugMorr', DUM1, DUM2, F11, F21, F12, F22
