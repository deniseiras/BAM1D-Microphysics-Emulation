...
!----------------------------------------------------------------------------
      subroutine taumol( &             !     call taumol( &
                        nlayers     , &!                nlayers                              , &
                        pavel       , &!                pavel        (1:nlayers)             , &
                        wx          , &!                wx           (1:maxxsec,1:nlayers)   , &
                        coldry      , &!                coldry       (1:nlayers)             , &
                        laytrop     , &!                laytrop                              , &
                        jp          , &!                jp           (1:nlayers)             , &
                        jt          , &!                jt           (1:nlayers)             , &
                        jt1         , &!                jt1          (1:nlayers)             , &
                        planklay    , &!                planklay     (1:nlayers,1:nbndlw)    , &
                        planklev    , &!                planklev     (0:nlayers,1:nbndlw)    , &
                        plankbnd    , &!                plankbnd     (1:nbndlw)              , &
                        colh2o      , &!                colh2o       (1:nlayers)             , &
                        colco2      , &!                colco2       (1:nlayers)             , &
                        colo3       , &!                colo3        (1:nlayers)             , &
                        coln2o      , &!                coln2o       (1:nlayers)             , &
                        colco       , &!                colco        (1:nlayers)             , &
                        colch4      , &!                colch4       (1:nlayers)             , &
                        colo2       , &!                colo2        (1:nlayers)             , &
                        colbrd      , &!                colbrd       (1:nlayers)             , &
                        fac00       , &!                fac00        (1:nlayers)             , &
                        fac01       , &!                fac01        (1:nlayers)             , &
                        fac10       , &!                fac10        (1:nlayers)             , &
                        fac11       , &!                fac11        (1:nlayers)             , &
                        rat_h2oco2  , &!                rat_h2oco2   (1:nlayers)             , &
                        rat_h2oco2_1, &!                rat_h2oco2_1 (1:nlayers)             , &
                        rat_h2oo3   , &!                rat_h2oo3    (1:nlayers)             , &
                        rat_h2oo3_1 , &!                rat_h2oo3_1  (1:nlayers)             , &
                        rat_h2on2o  , &!                rat_h2on2o   (1:nlayers)             , &
                        rat_h2on2o_1, &!                rat_h2on2o_1 (1:nlayers)             , &
                        rat_h2och4  , &!                rat_h2och4   (1:nlayers)             , &
                        rat_h2och4_1, &!                rat_h2och4_1 (1:nlayers)             , &
                        rat_n2oco2  , &!                rat_n2oco2   (1:nlayers)             , &
                        rat_n2oco2_1, &!                rat_n2oco2_1 (1:nlayers)             , &
                        rat_o3co2   , &!                rat_o3co2    (1:nlayers)             , &
                        rat_o3co2_1 , &!                rat_o3co2_1  (1:nlayers)             , &
                        selffac     , &!                selffac      (1:nlayers)             , &
                        selffrac    , &!                selffrac     (1:nlayers)             , &
                        indself     , &!                indself      (1:nlayers)             , &
                        forfac      , &!                forfac       (1:nlayers)             , &
                        forfrac     , &!                forfrac      (1:nlayers)             , &
                        indfor      , &!                indfor       (1:nlayers)             , &
                        minorfrac   , &!                minorfrac    (1:nlayers)             , &
                        scaleminor  , &!                scaleminor   (1:nlayers)             , &
                        scaleminorn2, &!                scaleminorn2 (1:nlayers)             , &
                        indminor    , &!                indminor     (1:nlayers)             , &
                        fracs       , &!                fracs        (1:nlayers,1:ngptlw)    , &
                        taug          )!                taug         (1:nlayers,1:ngptlw)      )
!----------------------------------------------------------------------------
...
! ----- Input -----
      ...
      real(kind=r8), intent(in) :: pavel(1:nlayers)           ! layer pressures (mb) 
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: wx(1:maxxsec,1:nlayers)            ! cross-section amounts (mol/cm2)
                                                      !    Dimensions: (maxxsec,nlayers)
      real(kind=r8), intent(in) :: coldry(1:nlayers)           ! column amount (dry air)
                                                      !    Dimensions: (nlayers)
      ...
      integer, intent(in) :: jt1(1:nlayers)          !
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: planklay(1:nlayers,1:nbndlw)      ! 
                                                      !    Dimensions: (nlayers,nbndlw)
      real(kind=r8), intent(in) :: planklev(0:nlayers,1:nbndlw)     ! 
                                                      !    Dimensions: (nlayers,nbndlw)
      real(kind=r8), intent(in) :: plankbnd(1:nbndlw)        ! 
                                                      !    Dimensions: (nbndlw)

      real(kind=r8), intent(in) :: colh2o(1:nlayers)          ! column amount (h2o)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: colco2(1:nlayers)          ! column amount (co2)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: colo3(1:nlayers)           ! column amount (o3)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: coln2o(1:nlayers)          ! column amount (n2o)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: colco(1:nlayers)           ! column amount (co)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: colch4(1:nlayers)          ! column amount (ch4)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: colo2(1:nlayers)           ! column amount (o2)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: colbrd(1:nlayers)          ! column amount (broadening gases)
                                                      !    Dimensions: (nlayers)
      ...
      integer, intent(in) :: indminor(1:nlayers)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: minorfrac(1:nlayers)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: scaleminor(1:nlayers)
                                                      !    Dimensions: (nlayers)
      real(kind=r8), intent(in) :: scaleminorn2(1:nlayers)
                                                      !    Dimensions: (nlayers)
...
! ----- Output -----
      real(kind=r8), intent(out) :: fracs(1:nlayers,1:ngptlw)         ! planck fractions
                                                      !    Dimensions: (nlayers,ngptlw)
      real(kind=r8), intent(out) :: taug(1:nlayers,1:ngptlw)         ! gaseous optical depth 
                                                      !    Dimensions: (nlayers,ngptlw)
...
! Calculate gaseous optical depth and planck fractions for each spectral band.
      ...
      call taugb1(nlayers,ngptlw,laytrop,fracs  ,taug   )
      call taugb2(nlayers,ngptlw,laytrop,fracs  ,taug   )
      call taugb3(nlayers,ngptlw,laytrop,fracs  ,taug   )
      call taugb4(nlayers,ngptlw,laytrop,fracs  ,taug   )
      call taugb5(nlayers,ngptlw,laytrop,fracs  ,taug   )
      call taugb6(nlayers,ngptlw,laytrop,fracs  ,taug   )
      call taugb7(nlayers,ngptlw,laytrop,fracs  ,taug   )
      call taugb8(nlayers,ngptlw,laytrop,fracs  ,taug   )
      call taugb9(nlayers,ngptlw,laytrop,fracs  ,taug   )
      call taugb10(nlayers,ngptlw,laytrop,fracs  ,taug   )
      call taugb11(nlayers,ngptlw,laytrop,fracs  ,taug   )
      call taugb12(nlayers,ngptlw,laytrop,fracs  ,taug   )
      call taugb13(nlayers,ngptlw,laytrop,fracs  ,taug   )
      call taugb14(nlayers,ngptlw,laytrop,fracs  ,taug   )
      call taugb15(nlayers,ngptlw,laytrop,fracs  ,taug   )
      call taugb16(nlayers,ngptlw,laytrop,fracs  ,taug   )

      contains

!----------------------------------------------------------------------------
      subroutine taugb1(nlayers,ngptlw,laytrop,fracs  ,taug   )
!----------------------------------------------------------------------------
...
! ------- Declarations -------
      ...
      integer      , intent(in ) :: laytrop         ! tropopause layer index
      real(kind=r8), intent(out) :: fracs  (1:nlayers,1:ngptlw)         ! planck fractions!    Dimensions: (nlayers,ngptlw)
      real(kind=r8), intent(out) :: taug   (1:nlayers,1:ngptlw)         ! gaseous optical depth !    Dimensions: (nlayers,ngptlw)
...
!----------------------------------------------------------------------------
      subroutine taugb2(nlayers,ngptlw,laytrop,fracs  ,taug   )
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taugb3(nlayers,ngptlw,laytrop,fracs  ,taug   )
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taugb4(nlayers,ngptlw,laytrop,fracs  ,taug   )
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taugb5(nlayers,ngptlw,laytrop,fracs  ,taug   )
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taugb6(nlayers,ngptlw,laytrop,fracs  ,taug   )
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taugb7(nlayers,ngptlw,laytrop,fracs  ,taug   )
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taugb8(nlayers,ngptlw,laytrop,fracs  ,taug   )
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taugb9(nlayers,ngptlw,laytrop,fracs  ,taug   )
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taugb10(nlayers,ngptlw,laytrop,fracs  ,taug   )
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taugb11(nlayers,ngptlw,laytrop,fracs  ,taug   )
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taugb12(nlayers,ngptlw,laytrop,fracs  ,taug   )
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taugb13(nlayers,ngptlw,laytrop,fracs  ,taug   )
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taugb14(nlayers,ngptlw,laytrop,fracs  ,taug   )
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taugb15(nlayers,ngptlw,laytrop,fracs  ,taug   )
!----------------------------------------------------------------------------
!
...
!----------------------------------------------------------------------------
      subroutine taugb16(nlayers,ngptlw,laytrop,fracs  ,taug   )
!----------------------------------------------------------------------------
!
