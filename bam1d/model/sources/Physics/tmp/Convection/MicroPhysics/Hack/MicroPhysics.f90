...
!     REAL(r8), POINTER :: lnpintdry(:,:,:) !(pcols,pver+1)! log interface pressure dry (Pa) 
  ...
  PUBLIC :: DestroyMicroPhysics

CONTAINS
  SUBROUTINE InitMicroPhysics(kMax,jMax,ibMax,jbMax,ppcnst,si_in,sl_in,delsig_in)
    IMPLICIT NONE
...
!   end if
  ...
  SUBROUTINE stratiform_tend(&
    ...
    REAL(r8), INTENT(inout)  :: state_t2  (pcols,pver)
    REAL(r8), INTENT(inout)  :: state_t          (pcols,pver)
    REAL(r8), INTENT(inout)  :: state_qv2 (pcols,pver)
    REAL(r8), INTENT(inout)  :: state_qv  (pcols,pver)
    REAL(r8), INTENT(inout)  :: state_ql2 (pcols,pver)
    REAL(r8), INTENT(inout)  :: state_ql  (pcols,pver)
    REAL(r8), INTENT(inout)  :: state_qi2 (pcols,pver)
    ...
    pnot      (1:pcols)       = 1.0e5_r8!state_ps  (1:pcols) 
    ...
    !
    pmid(1:pcols,1:pver) = state1%pmid  (1:pcols,1:pver,latco)
    t   (1:pcols,1:pver) = state1%t     (1:pcols,1:pver,latco)
    q   (1:pcols,1:pver) = state1%q     (1:pcols,1:pver,latco,1)
    omga(1:pcols,1:pver) = state1%omega (1:pcols,1:pver,latco)
    pdel(1:pcols,1:pver) = state1%pdel  (1:pcols,1:pver,latco)
    ps  (1:pcols)        = state1%pint  (1:pcols,pverp,latco)
    phis(1:pcols)        = state1%phis  (1:pcols,latco)
    ...
    snow_sed(1:pcols) = snow_sed(1:pcols)/1000.0_r8
    rain    (1:pcols) = rain(1:pcols)/1000.0_r8
    ! compute total precip (m/s)
    prec_sed(1:pcols) = rain(1:pcols) + snow_sed(1:pcols)
    ...
    prec_str(1:pcols) = prec_sed(1:pcols)
    snow_str(1:pcols) = snow_sed(1:pcols)
...
!!$          ptend_loc%q(i,k,ixcldliq) = dlf(i,k) * cld(i,k)
    ...
    prec_str(1:pcols) = prec_str(1:pcols) - rliq(1:pcols)  ! ( snow contribution is zero )
...
!!$    buffld2 => pbuf(ixbuffld2)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    ...
    totcw(1:pcols,1:pver) = state1%q(1:pcols,1:pver,latco,ixcldice) + state1%q(1:pcols,1:pver,latco,ixcldliq)

    ! save input cloud ice
    repartht(1:pcols,1:pver) = state1%q(1:pcols,1:pver,latco,ixcldice)

    ! Repartition ice and liquid
    !  state1%q(1:pcols,1:pver,ixcldice) = totcw(1:pcols,1:pver) * fice(1:pcols,1:pver)
    !  state1%q(1:pcols,1:pver,ixcldliq) = totcw(1:pcols,1:pver) * (1.0_r8 - fice(1:pcols,1:pver))
    rdtime = 1.0_r8/dtime
    ptend_loc%q(1:pcols,1:pver,latco,ixcldice) = &
         ( totcw(1:pcols,1:pver) * fice(1:pcols,1:pver)            - state1%q(1:pcols,1:pver,latco,ixcldice) ) * rdtime
    ptend_loc%q(1:pcols,1:pver,latco,ixcldliq) = &
         ( totcw(1:pcols,1:pver) * (1.0_r8 - fice(1:pcols,1:pver)) - state1%q(1:pcols,1:pver,latco,ixcldliq) ) * rdtime
    ...
    repartht(1:pcols,1:pver) = latice/dtime * (state1%q(1:pcols,1:pver,latco,ixcldice) - repartht(1:pcols,1:pver))

    ! calculate the tendencies for moisture, temperature and cloud fraction
    qtend(1:pcols,1:pver) = (state1%q(1:pcols,1:pver,latco,1) - qcwat(1:pcols,1:pver))*rdtime
    ttend(1:pcols,1:pver) = (state1%t(1:pcols,1:pver,latco)   - tcwat(1:pcols,1:pver))*rdtime
    ltend(1:pcols,1:pver) = (totcw  (1:pcols,1:pver)          - lcwat(1:pcols,1:pver))*rdtime
    ...
    snow_pcw  (1:pcols) = snow_pcw  (1:pcols)/1000.0_r8
    prec_pcw  (1:pcols) = prec_pcw  (1:pcols)/1000.0_r8
    !DO i = 1,pcols
...
!!$    ptend_loc%cflx_top = 0.
    ...
    ! accumulate prec and snow
    prec_str(1:pcols) = prec_str(1:pcols) + prec_pcw(1:pcols)
    snow_str(1:pcols) = snow_str(1:pcols) + snow_pcw(1:pcols)
    ...
    DO k=1,pver
       qcwat(1:pcols,k) = state1%q(1:pcols,k,latco,1)
       tcwat(1:pcols,k) = state1%t(1:pcols,k,latco)
       lcwat(1:pcols,k) = state1%q(1:pcols,k,latco,ixcldice) + state1%q(1:pcols,k,latco,ixcldliq)
    END DO
  ...
  SUBROUTINE physics_update(state, tend, ptend, dt,ppcnst,pcols,pver,latco)
    ...
    DO m = 1, ppcnst
       IF(ptend%lq(latco,m)) THEN
          ...
          CALL qneg3(  pcols, pcols, pver, m, m, qmin(m), qq(1,1,m))
...
!!$      wt = min(1.0_r8,max(0.0_r8,(t0-t(i,k))*rdtice))
       ...
       capn = capnw + (capnc-capnw) * MIN(1.0_r8,MAX(0.0_r8,1.0_r8-(p(i,k)-0.8_r8*p(i,pver))/(0.2_r8*p(i,pver))))
...
!    conke  = 50.e-6_r8    ! tunable constant for evaporation of precip
  ...
  SUBROUTINE cld_sediment_vel (ncol, pcols,  pver, pverp,      &
    ...
    ! compute air density
    rho(1:ncol,:) = pmid(1:ncol,:) / (rair * t(1:ncol,:))
...
!!$             vfall = 122.63 + 44.111*logiwc + 4.2144*logiwc**2
  ...
  SUBROUTINE cld_sediment_tend (ncol, pcols,pver,pverp, dtime  ,               &
    ...
    fxliq  (1:ncol,:) = 0.0_r8 ! flux at interfaces (liquid)
    fxice  (1:ncol,:) = 0.0_r8 ! flux at interfaces (ice)
    liqtend(1:ncol,:) = 0.0_r8 ! condensate tend (liquid)
    icetend(1:ncol,:) = 0.0_r8 ! condensate tend (ice)
    wvtend(1:ncol,:)  = 0.0_r8 ! environmental moistening
    htend(1:ncol,:)   = 0.0_r8 ! evaporative cooling
    sfliq(1:ncol)     = 0.0_r8 ! condensate sedimentation flux out bot of column (liquid)
    sfice(1:ncol)     = 0.0_r8 ! condensate sedimentation flux out bot of column (ice)
    ...
    DO k = 2,pver
       fxliq(1:ncol,k) = MAX(0.0_r8, fxliq(1:ncol,k))
       fxice(1:ncol,k) = MAX(0.0_r8, fxice(1:ncol,k))
    END DO
...
!!$        fxice(i,k+1) = min( fxice(i,k+1), cloud(i,k) * cldice(i,k) * pdel(i,k) )
    ...
    sfliq(1:ncol) = fxliq(1:ncol,pverp) / (dtime*gravit)
    sfice(1:ncol) = fxice(1:ncol,pverp) / (dtime*gravit)
  ...
  SUBROUTINE cldfrc_init(pver,plat)
    ...
    ENDIF
          ...
          rhminl = .95_r8     ! minimum rh for low stable clouds
          rhminh = .70_r8     ! minimum rh for high stable clouds
          sh1 = 0.07_r8       ! parameters for shallow convection cloud fraction
          sh2 = 500.0_r8      ! parameters for shallow convection cloud fraction
          dp1 = 0.14_r8       ! parameters for deep convection cloud fraction
  ...
  SUBROUTINE cldfrc( &
    ...
    land(1:pcols) = NINT(landfrac(1:pcols)) == 1
...
10  DO i=1,len
  ...
  SUBROUTINE DestroyMicroPhysics()
    IMPLICIT NONE

  END SUBROUTINE DestroyMicroPhysics
...
! REAL(r8) :: prec_pcw     (ibMax)  ! sfc flux of precip from microphysics(m/s)
! REAL(r8) :: snow_pcw     (ibMax)  ! sfc flux of snow from microphysics (m/s)
...
! CALL  InitMicroPhysics(kMax,jMax,ibMax,jbMax,ppcnst,si,sl,delsig)
...
!      prec_pcw    , &! REAL(r8), INTENT(out)  :: prec_pcw    (ibMax)  ! sfc flux of precip from microphysics(m/s)
!      snow_pcw    , &! REAL(r8), INTENT(out)  :: snow_pcw    (ibMax)  ! sfc flux of snow from microphysics (m/s)
