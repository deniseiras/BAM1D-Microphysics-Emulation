...
MODULE GwddDriver
  ...
  USE Options, ONLY: &
       reducedGrid,igwd,nfvar,fNameOrgvar,fNameGtopog,nfprt
...
CONTAINS
  SUBROUTINE InitGWDDDriver(ibMax,jbMax,iMax,jMax,kmax,  si,sl,del,grid)
   IMPLICIT NONE
    ...
    CALL InitVariancia(iMax,jMax,igwd,nfvar,fNameOrgvar,fNameGtopog,grid)
 ...
 SUBROUTINE InitVariancia(iMax,jMax,igwd,nfvar,fNameOrgvar,fNameGtopog,grid)
    INTEGER          , INTENT(IN   ) :: iMax
    INTEGER          , INTENT(IN   ) :: jMax
    ...
    OPEN (UNIT=nfvar,FILE=TRIM(fNameOrgvar),FORM='FORMATTED', ACCESS='SEQUENTIAL', &
    ...
    IF(TRIM(igwd).EQ.'YES' .or. TRIM(igwd).EQ.'CAM' .or. TRIM(igwd).EQ.'USS') THEN
       irec=1
       CALL ReadVar(nfvar,irec,var_in,0,'txt')
       IF (reducedGrid) THEN
    ...
    IF( TRIM(igwd) == 'USS') THEN
       ...
       buffer  =0.0_r4
       INQUIRE (IOLENGTH=LRecIn) buffer
       OPEN (UNIT=nfvar,FILE=TRIM(fNameGtopog),FORM='FORMATTED', ACCESS='SEQUENTIAL', &
         ACTION='READ',STATUS='OLD', IOSTAT=ierr)
       ...
       irec=1
       CALL ReadVar(nfvar,irec,var_in,1,'txt')
       !IF (reducedGrid) THEN
  ...
  SUBROUTINE Gwdd_Driver (ps ,gu   ,gv   ,gt   , gq,chug, chvg, chtg,xdrag, ydrag, &
            ...
            kmax,latco,dt,imask,colrad,topog)
   IMPLICIT NONE
