...
CONTAINS
  ...
  SUBROUTINE lsxmain(ginvap       , &! INTENT(OUT  )!local
    ...
    DO i = 1, npoi
       !
       qh(i)=0.622e0_r8*EXP(21.65605e0_r8 -5418.0e0_r8 /tg(i))/(psurf(i)/100.0_r8)
  ...
  SUBROUTINE soilh2o (owsoi   , &! INTENT(IN   )
    ...
    REAL(KIND=r8)    :: zz
    ! 
    REAL(KIND=r8)    :: hsoim(nsoilay+1)  ! vertical distances between centers of layers
    !
    ...
    DO k = 2, nsoilay
       ...
       DO i = 1, npoi
          !
          hsoim(k) = 0.5_r8 * (hsoi(k-1) + hsoi(k))
          !
          weim(i,k) = 0.5_r8 * hsoi(k) / hsoim(k)
    ...
    DO k = 1, nsoilay
       ...
       DO  i = 1, npoi
          ...
          d1(i,k) = dt*(   f(i,k)*0.5_r8*hsoi(k)/hsoim(k)    &
               - g(i,k)/hsoim(k) )
       ...
       IF (k.LT.nsoilay) THEN
          ...
          DO i=1,npoi
             !
             dt = dtime / (porosflo(i,k)*hsoi(k))
             d2(i,k) = 1.0_r8 + dt*( - f(i,k+1)*0.5_r8*hsoi(k+1)/hsoim(k+1)  &
                  + f(i,k)  *0.5_r8*hsoi(km1)/hsoim(k)    &
                  + g(i,k+1)/hsoim(k+1)                & 
                  + g(i,k)  /hsoim(k) )
             d3(i,k) = dt*( - f(i,k+1)*0.5_r8*hsoi(k)/hsoim(k+1)         &
                  - g(i,k+1)            /hsoim(k+1) )
             !
          END DO
          !
    ...
    DO k = nsoilay, 1, -1
       ...
       DO i = 1, npoi
          !
          zz = rhow * poros(i,k) *    &
               MAX(epsilon, (1.0_r8-wisoi(i,k))) * hsoi(k)
          !
  ...
  SUBROUTINE soilheat (otsoi  , &! INTENT(IN   )
    ...
    DO k = 2, nsoilay
       ...
       DO i = 1, npoi
          !
          con(i,k) =  1.0_r8 / (0.5_r8 * (hsoi(k-1) / consoi(i,k-1) + &
               hsoi(k)   / consoi(i,k)))
          !
  ...
  SUBROUTINE canopy(tfac         , &! INTENT(INOUT) !local
    ...
    REAL(KIND=r8) :: cdmaxa, cdmaxb, ctau
    INTEGER :: i
  ...
  SUBROUTINE drystress(froot     , &! INTENT(IN   )
    ...
    DO i = 1, npoi
       ...
       DO k = 1, nsoilay
          ...
          !
          stressl(i,k) = froot(k,1) * MAX (0.0_r8, MIN (1.0_r8, zwilt))
          stressu(i,k) = froot(k,2) * MAX (0.0_r8, MIN (1.0_r8, zwilt))
          !
  ...
  SUBROUTINE turvap(iter     , &! INTENT(IN   )
    ...
    IF (iter.EQ.1) THEN
       ...
       !
       DO i = 1, npoi
          cog(i) = consoi(i,1) / (0.5_r8 * hsoi(1))
