...
CONTAINS
  ...
  SUBROUTINE InitSfc_MellorYamada0(kmax, sig, delsig, sigml)
  ...
  SUBROUTINE ympbl20 ( &
       gu    ,gv    ,gt    ,gq    ,delsig,ncols , &
       kmax  ,delt  ,colrad ,gmt   ,gmq   ,gmu   ,gl0 ,PBL_CoefKm,PBL_CoefKh,pblh,tkemyj,gice,gliq)
    !
    !
    ...
    !     
    IF (delt == deltx) THEN
       WRITE(UNIT=nfprt, FMT="(' ERROR - delt == 0  in ympbl20_SFC',2f12.6 )")delt , deltx
       STOP "**(ymbpl0)"
    END IF
       DO i=1,ncols,1 
...
!             tkemyj(i,k)=Pbl_EddEner(i,K)**2 
  ...
  !XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
  !---------------------------------------------------------------------
  SUBROUTINE VDIFHM(kMax,nCols,twodti,Pbl_ITemp,Pbl_CoefKh,a,b,gice ,gliq ,gmq)
    !     ***************************************************************
    !     *                                                             *
    !     *         VERTICAL DIFFUSION OF MASS VARIABLES                *
    !     *                                                             *
    ...
    REAL(KIND=r8) :: Pbl_DifQms (nCols,kMax,3) 
    REAL(KIND=r8) :: Pbl_TendQ  (nCols,kMax,3) !1/(sec * K)
